...
kbasprs.neye = 0; %size(sta,1); % Number of "identity" basis vectors near time of spike;
kbasprs.ncos = nk+1; % Number of raised-cosine vectors to use  
...
last = [ktbas(:,1); ktbas(end,1); flipud(ktbas(:,1))];
ktbas = ktbas(:,2:end);
gg.ktbas = ktbas;
gg.ktbas = zeros(size(ktbas,1)*2+1, size(ktbas,2)*2+1);
gg.ktbas(1:nkt, 1:nk) = ktbas;
gg.ktbas((nkt+2):(2*nkt+1),(nk+1):(2*nk)) = flipud(ktbas);
gg.ktbas(:,end) = last/norm(last);
gg.kbasprs = kbasprs;
gg.ktbas = gg.ktbas((nkt/2+1):(3*nkt/2+1),:);
