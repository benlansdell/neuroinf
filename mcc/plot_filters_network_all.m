function plot_filters_network_all(models, processed, fn_out, goodunits)
	...
	for i = 1:nU
		...
		for j = 1:(nU)
			...
			if j == i
				...
				if j == 1
					...
					title(['Unit: ' num2str(goodunits(i)) ' ' name]);
				end
			else
				if j > i
					jp = j-1;
				...
				if i == 1
					title(['Unit: ' num2str(goodunits(j)) ' ' name]);
				end
				if i == nM
					xlabel('time (ms)');
				end
		...
		str1(1) = {['Unit no.: ' num2str(goodunits(i)) ' Unit: ' name]};
