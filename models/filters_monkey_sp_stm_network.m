function data = filters_monkey_sp_stm_network(processed, nK_sp, nK_stm, dt_sp, dt_stm)
	...
	data.k{nU+1,2} = (1:(nK_stm)) + nK_sp;
	data.k{nU+1,3} = dt_stm;
	data.k{nU+2,1} = 'curs y';
	data.k{nU+2,2} = (1:(nK_stm)) + nK_sp + nK_stm;
	data.k{nU+2,3} = dt_stm;
	data.k{nU+3,1} = 'curs z';
	data.k{nU+3,2} = (1:(nK_stm)) + nK_sp + 2*nK_stm;
	data.k{nU+3,3} = dt_stm;
	data.k{nU+4,1} = 'grip';
	data.k{nU+4,2} = (1:(nK_stm)) + nK_sp + 3*nK_stm;
