function data = filters_monkey_sprc_stm_pca(processed, nK_sp, nK_stm, a, b, dt_sp, dt_stm)
	%Prepare spike and stimulus data for GLM
	%
	%Usage:
	%	data = filters_monkey_sp_stm_pca(processed, nK_sp, nK_stm)
	%     
	%Input:
	...
	%	a = (optional, default = 15) rate at which raised cosine functions spread out
	%	b = (optional, default = 10) number of principle components in cursor and grip data to keep 
	...
	if (nargin < 5) b = 10; end
	if (nargin < 6) dt_sp = processed.binsize; end
	if (nargin < 7) dt_pos = processed.binsize; end
	...
	%Look at svd of a subset of the stim data, pull out principle components, and project the data onto these
	ii_stim = (nU*nK_rc+1):nK;
	stim = data.X(:,ii_stim);
	cov = stim'*stim;
	[e, l] = eigs(cov, b);
	stim_red = stim*e;
	data.X = [data.X(:,data.sp_hist), stim_red];
