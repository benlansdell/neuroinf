...
for l = 3:length(lambdas)
    l
    lambda = lambdas(l);
    r1 = load([wd fn_out 'GLM_coupled_simulation_L1_popcond_lambda_' num2str(lambda) '_ID_1.mat']);
    %Check if Rt_glm needs normalizing
    Rt_glm = {};
    for idx = 1:nU
        Rt_glm{idx} = sum(r1.Rt_glm_popcond{idx},1);
    end
    nB = size(Rt_glm{1}, 2);
    ...
    save([wd fn_out '/preprocessed_networkglm_sims_popcond_' num2str(lambda) '.mat'], 'proc_withheld', 'nU', 'Rt_glm', 'goodunits', 'RefreshRate')
    coh_out = ['popcond_coherence_lambda_' num2str(lambda)];
    fn_ins{l-2} = ['/preprocessed_networkglm_sims_popcond_' num2str(lambda) '.mat'];
    jackknifecoherence([wd fn_out], ['/preprocessed_networkglm_sims_popcond_' num2str(lambda) '.mat'], coh_out)
end
...
title('Population conditioned simulation')
saveplot(gcf, [wd fn_out '/GLM_loglikelihood_popcond_compare_semilog.eps'])

%Find max log-likelihood for each unit
maxlogl = max(logl_glm(3:end-1,:), [], 1);
%uncoupledlogl = logl_glm(end,:);
...
title('Population conditioned simulation')
saveplot(gcf, [wd fn_out '/GLM_loglikelihood_popcond_compare.eps'])
...
ylabel('Coupled log-likelihood')
title('Population conditioned simulation')
saveplot(gcf, [wd fn_out '/GLM_loglikelihood_popcond_lambda.eps'], 'eps', [7 4])
...
title('Population conditioned simulation')
saveplot(gcf, [wd fn_out '/GLM_loglikelihood_popcond_lambda_relative.eps'], 'eps', [7 4])
