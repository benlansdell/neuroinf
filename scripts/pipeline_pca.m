...
frames = 160;                    %no. stim frames 
nF = 2*frames+1;
nK = 10;                        %no. stim PC components to keep
...
nB = size(proc.stim, 1);
fn_out = './PCAfilters_individPCA.eps';
...
for icell = 1:nU
    ...
    for idx = 1:nS
        ...
        C = cov(stacked(:,ii)-repmat(mean(stacked(:,ii),1),[nB,1]));
        [v,eC] = eig(C);
    ...
    gg0 = makeFittingStruct_GLM_monkey(sta, dt, vC(:,1:(nK*nS)));
    gg0.tsp = resp';
    gg0.tspi = 1;
