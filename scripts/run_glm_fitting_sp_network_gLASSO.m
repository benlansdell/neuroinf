...
fn_out = './run_glm_fitting_sp_network_glasso.eps';
...
for idx = 1:length(responsefiles)
	...
	models{idx} = MLE_SD_grouplasso(data, lambda, const);
end

%Save models, and other data
data = rmfield(data, {'X', 'y'});
processed = rmfield(processed, {'stim', 'spikes'});
save('./run_glm_fitting_sp_network_glasso.mat', 'models', 'data', 'processed')
