...
processed = preprocess_monkey_pillow_10Hz(datafile, binsize, dt, frames);    
...
p = frames*nS;
fn_out = './monkeyresults/run_glm_fitting_pillow_10Hz_trim.eps';
ggs = {};
for icell = 1:nU
    disp(num2str(icell));
    stim = processed.stim;
    ...
    save(['./monkeyresults/Pillow_cell_' num2str(icell) '_glm_ext_10Hz_trim.mat'],...
        'gg'); 
    ggs{icell} = gg;
end
...
for icell = 1:nU
    ...
    stim = stim/p;
    load(['./monkeyresults/Pillow_cell_' num2str(idx) '_glm_ext_10Hz_trim.mat']);
    ...
    save(['GLM_model_' num2str(idx) '_ext_10Hz_trim.mat'],'gg','Rt_glm');
end
...
fn_out = './monkeyresults/run_glm_fitting_pillow_10Hz_compare_trim.eps';
processed_pillow = processed;
load('./monkeyresults/run_glm_fitting_sp_100Hz_forwardback_trim_80pt.mat');
ggs = {};
for icell = 1:nU
    load([['./monkeyresults/Pillow_cell_' num2str(icell) '_glm_ext_10Hz_trim.mat']])
    ggs{icell} = gg;
end
...
unitidx = 7;
unit = t(unitidx,2);
load(['GLM_model_' num2str(unit) '_ext_10Hz_trim.mat'],'gg','Rt_glm');
tidx = tstart:tend;
