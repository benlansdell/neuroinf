...
fn_out = './results2_retina_20maxit_6frame_sansoffset_trim';
mkdir(fn_out);
...
for icell = 1:3
    for iL = 1:2
        ...
        [gg, negloglival] = MLfit_GLM_trim(gg0,S1,opts, processed, trim, pcas, offset);
        Rt_glm = zeros(1,Tt) ;
        for ir = 1:rep
            [iR_glm, vmem,Ispk] = simGLM_monkey(gg, S1t, time_limit, offset) ;
            Rt_glm(ceil(iR_glm)) = Rt_glm(ceil(iR_glm))+1 ;
        end
        Rt_glm = Rt_glm'/rep + 1e-8 ;
        save([fn_out '/Retina_cell_' num2str(icell) '_glmamended_' stim_length{iL} '.mat'],'gg','Rt_glm') ; 
        logl_glm(iL,icell) = mean(Rt.*log(Rt_glm)-(Rt_glm)*dt) ;
    end
end
save([fn_out '/Retina_GLMamedned_model_All_LogLikelihood.mat'],'logl_glm') ;

%Plot filters and plot simulation results
makefig_RetinaData_GLM(fn_out)