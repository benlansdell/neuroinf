function jackknifecoherence 
	...
	for icell = 1:nU
	    ...
	    subplot(nU,1,icell)
		hold on
		%area(f(ii), ginv((Coh_cpl(ii))+2*Coh_cpl_SE(ii)), 'FaceColor', [0.7 .7 .7])
		%area(f(ii), ginv((Coh_cpl(ii))-2*Coh_cpl_SE(ii)), 'FaceColor', [1 1 1])
		%area(f(ii), ginv((Coh(ii))+2*Coh_SE(ii)), 'FaceColor', [0.7 .7 .7])
		%area(f(ii), ginv((Coh(ii))-2*Coh_SE(ii)), 'FaceColor', [1 1 1])
		%area(f(ii), Coh_cpl(ii)+1*Coh_cpl_SE(ii), 'FaceColor', [0.7 .7 .7])
		%area(f(ii), Coh_cpl(ii)-1*Coh_cpl_SE(ii), 'FaceColor', [1 1 1])
		%area(f(ii), Coh(ii)+1*Coh_SE(ii), 'FaceColor', [0.7 .7 .7])
		%area(f(ii), Coh(ii)-1*Coh_SE(ii), 'FaceColor', [1 1 1])
	    %h1 = plot(f(ii), (Coh(ii)), 'Color', [0 0 0.6]);
	    %h2 = plot(f(ii), (Coh_cpl(ii)), 'Color', [0 .6 0]);

		h1 = shadedErrorBar(f(ii), Coh(ii), Coh_SE(ii), '-b', 0);
		h2 = shadedErrorBar(f(ii), Coh_cpl(ii), Coh_cpl_SE(ii), '-g', 0);

	    plot(f(ii), Null*ones(size(ii)), 'r');
	    legend([h1.mainLine; h2.mainLine], {'Uncoupled', 'Coupled'})
