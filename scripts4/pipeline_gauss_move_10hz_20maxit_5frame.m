...
for icell = 1:nU
    ...
    for i = 1:runs
        ...
        [coh_cpl, k] = coherence_hyp(truesp{i}, simsp_cpl{i}, N_sample_max);
        coh = coherence_hyp(truesp{i}, simsp{i}, N_sample_max);
        Coh_cpl(i,:) = coh_cpl;
        Coh(i,:) = coh;
        K(i) = k;
    end
    ...
    DF = sum(~isnan(Coh(:,1)))*K-1; Null = sqrt(1-0.05^(1/DF));
    L = 5; 
    clf
    ii = 1:fix(Pad/L);
    plot(f(ii), abs(Coh(ii)), f(ii), abs(Coh_cpl(ii)), f(ii), Null*ones(size(ii)))
    legend('Uncoupled', 'Coupled')
    ylabel('<|coh|>')
    saveplot(gcf, [fn_out '/GLM_coherence_' num2str(goodunits(icell)) '_inmovement.eps'])
end
